...
public abstract class AbstractPermissionVisitor extends BaseVisitor {
  ...
  public void visit(Source source) {
     ...
     */
    Map<String, Object[][]> permissionConditionMap = getPermissionConditionMap();
  ...
  private void addPermission(String tableName, String tableAlias, Conditions conditions,
      boolean[] flag, Map<String, Object[][]> permissionConditionMap) {
    if (StringUtils.isEmpty(tableName) || !permissionConditionMap.containsKey(tableName)) {
      return;
    }
    Object[][] objPairs = permissionConditionMap.get(tableName);
   ...
   *
   * 另外，也可以在实现类中使用ConditionsBuilder，拼接出复杂的查询条件。在这种情况下，value[0]可以为null，value[1]为复杂的Conditions对象。
   * 详见示例！
   *
   * @return
   */
  public abstract Map<String, Object[][]> getPermissionConditionMap();
