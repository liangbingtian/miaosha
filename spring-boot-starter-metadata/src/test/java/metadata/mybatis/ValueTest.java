...
public class ValueTest {
  ...
  public void valueTest() {
    ...
    Assert.assertEquals(
        "(#{_p1},#{_p2},#{_p3},#{_p4},#{_p5})",
        sqlVisitor.getSql());
    Assert.assertEquals("{_p2=2, _p1=1, _p4=4, _p3=3, _p5=5}", sqlVisitor.getMybatisParamMap().toString());

    value = Utils.getTObject("value/value2.json", Value.class);
    sqlVisitor.reset();
    sqlVisitor.visit(value);
    Assert.assertEquals("{_p2=b, _p1=a, _p4=d, _p3=c, _p5=e}", sqlVisitor.getMybatisParamMap().toString());
    Assert.assertEquals(
        "(#{_p1},#{_p2},#{_p3},#{_p4},#{_p5})",
        sqlVisitor.getSql());
  }
